...
def extract_and_write_h5(bag_path: Path, cfg: dict, out_root: Path):
    ...
    if topic_map.get("scan") in synced:
        ...
        for m in synced[topic_map["scan"]]:
            if m is None:
                ...
                continue
            arr = np.array(m.ranges, dtype=np.float32, copy=True)
            if arr.size < 1080:
        ...
        scan = np.stack(scan_list, axis=0)
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        if control is not None:
            f.create_dataset("control", data=control, dtype="f4", **filter_args)
        ...
        if velocity is not None:
            f.create_dataset("velocity", data=velocity, dtype="f4", **filter_args)
        if pose is not None:
            f.create_dataset("pose", data=pose, dtype="f4", **filter_args)
        if imu is not None:
            f.create_dataset("imu", data=imu, dtype="f4", **filter_args)
        if gnss is not None:
        ...
        grp.create_dataset("index", data=idx.astype(np.int64), **filter_args)
