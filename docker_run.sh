...
rocker ${opts} --x11 --devices /dev/dri --group-add rdma --user --net host --privileged --name "aichallenge-2025-$(date "+%Y-%m-%d-%H-%M-%S")" --volume ${volume} --volume /dev/input/:/dev/input -- "aichallenge-2025-${target}" 2>&1 | tee "$LOG_FILE"